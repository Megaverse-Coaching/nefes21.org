...
var KTCategoryList = function () {
    ...
    var initCategoryTable = function () {
        ...
        // Select2 ayarları
        ['showcase_id','content_id'].forEach(function(selector) {
            $(addForm.querySelector(`select[name="${selector}"]`)).select2();
        });

        // Checkbox değişimini yönetmek için yardımcı fonksiyon
        function handleCheckboxChange(checkboxName, checkedLabel, uncheckedLabel, labelSelector) {
            $(`input[type="checkbox"][name="${checkboxName}"]`).change(function () {
                ...
                $(labelSelector).text(isChecked ? checkedLabel : uncheckedLabel);
            });
        }
    ...
    var handleDeleteRows = () => {
        ...
        deleteButtons.forEach(d => {
            ...
            d.addEventListener('click', function (e) {
                ...
                // Get category name
                const recordName = parent.querySelectorAll('td')[5].querySelectorAll('a')[0].innerText;
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        axios.delete(d.getAttribute('href'), {today_showcase:recordName})
                ...
                if (result.value) {
                    ...
                    checkboxes.forEach(c => {
                        if (c.checked) {
                            ...
                            axios.delete("today-showcases/"+item, {today_showcase:item})
                                .then(function (response) {
                                    $("div.card").removeClass("overlay overlay-block");
                                    $(".overlay-layer").toggleClass("d-flex d-none");
                                    // console.log(response);
