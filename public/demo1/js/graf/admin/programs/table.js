...
var KTCategoryList = function () {
    ...
    var dimensionSelect = document.getElementById('dimensionMenu');
    ...
    var initCategoryTable = function () {
        ...
        $(dimensionSelect.querySelector('select[name="dimension"]')).select2().on('select2:select', function (e) {
            var data = e.params.data;
            if (data.id) {
                var baseURL = $(this).attr("data-redirect-url");
                window.location.replace(baseURL +"admin/showcase-pools/"+data.id);
            }
        });
        $(dimensionSelect.querySelector("select[name='dimension'] + span")).addClass('w-225px');
        ...
        datatable = $(table).DataTable({
            ...
            'columnDefs': [
                ...
                { orderable: false, targets: 7 }, // Disable ordering on column 4 (actions)
            ],
            "language" : {
    ...
    var handleDeleteRows = () => {
        ...
        deleteButtons.forEach(d => {
            ...
            d.addEventListener('click', function (e) {
                ...
                // Get category name
                const recordName = parent.querySelectorAll('td')[3].querySelectorAll('a')[0].innerText;
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        axios.delete(d.getAttribute('href'), {starter:recordName})
                ...
                if (result.value) {
                    ...
                    checkboxes.forEach(c => {
                        if (c.checked) {
                            ...
                            axios.delete("today-starters/"+item, {starter:item})
                                .then(function (response) {
                                    $("div.card").removeClass("overlay overlay-block");
                                    $(".overlay-layer").toggleClass("d-flex d-none");
                                    // console.log(response);
