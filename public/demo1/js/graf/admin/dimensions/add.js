...
var KTAddModal = function () {
    // Shared variables
    var table = document.getElementById('kt_table_dimensions');
    const element = document.getElementById('kt_modal_add');
    const form = element.querySelector('#kt_modal_add_form');
    const toolbarElement = document.getElementById('dimensionMenu');
    ...
    var initScripts = () => {
        ...
        $(form.querySelector('select[name="dimension_id"]')).select2();
        $(form.querySelector('select[name="content_id"]')).select2();

        $(toolbarElement.querySelector('select[name="dimension"]')).select2().on('select2:select', function (e) {
                var data = e.params.data;
                if (data.id) {
                    var baseURL = $(this).attr("data-redirect-url");
                    window.location.replace(baseURL +"admin/dimensions/"+data.id);
                }
            });
        $(toolbarElement.querySelector("select[name='dimension'] + span")).addClass('w-225px');
    }
