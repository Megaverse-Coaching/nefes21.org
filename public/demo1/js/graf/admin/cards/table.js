...
var KTCategoryList = function () {
    ...
    var dropDownSelect = document.getElementById('deckMenu');
    ...
    var description = document.getElementsByTagName('data-kt-autosize');
    ...
    var initCategoryTable = function () {
        ...
        $(dropDownSelect.querySelector('select[name="deck_id"]')).select2().on('select2:select', function (e) {
            var data = e.params.data;
            if (data.id) {
                var baseURL = $(this).attr("data-redirect-url");
                window.location.replace(baseURL +"admin/decks/"+data.id+"/cards");
            }
        });
        $(dropDownSelect.querySelector("select[name='deck_id'] + span")).addClass('w-225px');

        $('input[type=checkbox][name=daily]').change(function() {
            ($(this).is(':checked')) ? $(this).val(1).attr('checked', true): $(this).val(0).removeAttr('checked');
            ($(this).is(':checked')) ? $("span.status").html('Yes'): $("span.status").html('No');
        });
            ...
            d.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        axios.delete(d.getAttribute('href'), {card:recordName})
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    checkboxes.forEach(c => {
                        if (c.checked) {
                            var item = $(c).val();
                            baseURL = $(c).attr("data-redirect-url");

                            axios.delete(baseURL +"admin/cards/"+item, {card:item})
                                .then(function (response) {
                                    $("div.card").removeClass("overlay overlay-block");
                                    $(".overlay-layer").toggleClass("d-flex d-none");
                                    // console.log(response);
                                ...
                                .then(function () {
                                    Swal.fire({
                                        text: "Seçili İçeriklerin hepsi kaldırıldı!.",
                                        icon: "success",
                                        buttonsStyling: false,
                    ...
                    Swal.fire({
                        text: "Seçili kayıtlar silinmedi!.",
                        icon: "error",
                        buttonsStyling: false,
                        confirmButtonText: "Tamam, anladım!",
                        customClass: {
